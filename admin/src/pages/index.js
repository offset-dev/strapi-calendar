        ...
        <Box id="schedule" background="neutral0" shadow="filterShadow" padding={[5, 8]} hasRadius>
          <Flex justifyContent="space-between" style={{ marginBottom: 10 }}>
            <Flex>
              <IconButton noBorder
                onClick={() => setState(s => (
                  { ...s, date: moment(s.date).subtract(1, s.view.toLowerCase()).toISOString() }
                ))}
                icon={<ChevronLeft />} />
              <Box paddingLeft={1} paddingRight={1}>
                <DatePicker
                  ...
                  name="date"
                  aria-label={formatMessage({ id: getTrad('view.calendar.action.select-date') })}
                  value={moment(state.date).format('ll')}
                  onChange={e => setState(s => ({ ...s, date: moment(e).toISOString() }))} />
              </Box>
              <IconButton noBorder onClick={() => setState(s => (
                { ...s, date: moment(s.date).add(1, s.view.toLowerCase()).toISOString() }
              ))} icon={<ChevronRight />} />
              <Box>
                {settings.todayButton &&
                  <Button variant="secondary" size="L" onClick={() => setState(s => (
                    { ...s, date: moment().toISOString() }
                  ))}>
                    {formatMessage({ id: getTrad('view.calendar.action.today') })}
                  </Button>
                }
              </Box>
            </Flex>
          ...
          <Scheduler data={data} locale={formatMessage({ id: getTrad('view.calendar.locale') })} firstDayOfWeek={formatMessage({ id: getTrad('view.calendar.first-day-of-week') }) || 0}>
            <ViewState onCurrentDateChange={load}
              currentDate={moment(state.date).format()}
